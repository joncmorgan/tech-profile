...
class ResumeProjectTransformer:
  ...
  # Resolve paths for the repository structure:
    self.script_dir = Path(__file__).resolve().parent
    self.cv_root_dir = self.script_dir.parent
    self.repo_root_dir = self.cv_root_dir.parent
    self.projects_json_path = self.repo_root_dir / "data" / "projects.json"
    self.output_path = self.cv_root_dir / "data" / "master_experience.json"
  ...
  def run_single(self):
    ...
    processed_ids = {
        str(p.get("id")) for p in existing_transformed if "id" in p
    }

    unprocessed = [
        p
        for p in all_raw_projects
        if self.get_project_id(p) not in processed_ids
    ]

    if not unprocessed:
      print(
          "All projects are already processed for the resume! Use --menu to"
          " reprocess."
      )
      return
  ...
  def run_batch(self, force: bool = False):
    ...
    if force:
      print(
          "Force rebuild enabled: Re-processing ALL projects for resume from"
          " scratch."
      )
      existing_transformed = []
      ...
      if not targets:
        print("No pending resume projects to process.")
        return
      print(f"Processing {len(targets)} pending resume project(s)...")

    for i, project in enumerate(targets, 1):
      title = project.get("title", "Untitled")
      print(f"[{i}/{len(targets)}] Processing via Claude: '{title}'...")
      try:
        transformed = self.transform_project_with_claude(
  ...
  def run_interactive_menu(self):
    ...
    while True:
      existing_transformed = self.load_transformed_projects()
      processed_map = {
          str(p.get("id")): p for p in existing_transformed if "id" in p
      }

      print("\n--- Resume Project Transformation Menu ---")
      ...
      print("\nOptions:")
      print(
          "  [Number] : Enter project number to force re-process that specific"
          " project"
      )
      ...
      if choice == "q":
        ...
        confirm = (
            input(
                "Are you sure you want to force rebuild ALL resume projects?"
                " (y/n): "
            )
            .strip()
            .lower()
        ...
        if 0 <= idx < len(all_raw_projects):
          ...
          title = target.get("title", "Untitled")
          print(f"\nForce-processing '{title}' for resume...")

          try:
            prompt_template = self.load_prompt_template()
            new_transformed = self.transform_project_with_claude(
...
def main():
  parser = argparse.ArgumentParser(
      description="Transform projects.json into Resume format using Claude."
  )
  parser.add_argument(
      "--menu",
      action="store_true",
      help=(
          "Open an interactive menu to view status and reprocess specific"
          " resume projects."
      ),
  )
  parser.add_argument(
      "--force",
      action="store_true",
      help="Force rebuild all resume projects from scratch.",
  )
  args = parser.parse_args()

  transformer = ResumeProjectTransformer()
