...
def filter_master_resume(master_path: Path, experience_path: Path, profile_key: str = None, target_tags: list = None) -> dict:
    """Filters master_resume.json and merges master_experience.json into the standard schema."""
    with open(master_path, "r", encoding="utf-8") as f:
        master = json.load(f)

    # Load master_experience.json generated by projects_to_resume.py
    master_experience = []
    if experience_path.exists():
        with open(experience_path, "r", encoding="utf-8") as f:
            master_experience = json.load(f)
    else:
        print(f"⚠️ Warning: `master_experience.json` not found at `{experience_path}`. Experience summaries will be empty.")
    ...
    # 3. Build Experience Section (Merging master_resume framework with master_experience summaries)
    filtered_experience = []
    for exp in master.get("experience", []):
        company_name = exp.get("company", "")
        
        # Gather matching highlights/summaries from master_experience based on tags
        matching_highlights = []
        for item in master_experience:
            item_tags = item.get("tags", [])
            if matches_tags(item_tags):
                summary_text = item.get("summary", item.get("description", ""))
                if summary_text:
                    matching_highlights.append(summary_text)

        # Fallback to embedded master_resume highlights if master_experience didn't yield matches for this entry
        if not matching_highlights:
...
def generate_intermediate_json(cwd: Path, profile_key: str = None, target_tags: list = None) -> Path:
    """Locates master JSON and master experience JSON, runs filtering, and writes resume_data.json to CWD."""
    master_path = cwd / "master_resume.json"
    if not master_path.exists():
        master_path = DATA_DIR / "master_resume.json"

    experience_path = CV_DATA_DIR / "master_experience.json"
    if not experience_path.exists():
        experience_path = cwd / "master_experience.json"
    ...
    print(f"🔍 Processing Experience Data: `{experience_path.name}`")
    if profile_key:
        print(f"🎯 Target Profile        : `{profile_key}`")
    if target_tags:
        print(f"🏷️  Target Tags           : {target_tags}")

    filtered_data = filter_master_resume(master_path, experience_path, profile_key, target_tags)
